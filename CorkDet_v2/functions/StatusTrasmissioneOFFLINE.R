...
StatusTrasmissOFFLINE <- function(rawStazioni, rawStatusStaz,
                                  finestra,
  ...
  # crea colonna indice YYMMDDhhmm da DTRF
  for(i in 1:nrow(rawStazioni)){
    # i=1
    YYYY <- substr(rawStazioni$DTRF[i],7,10) #anno
    MM <- substr(rawStazioni$DTRF[i],4,5) #mese
    DD <- substr(rawStazioni$DTRF[i],1,2) #giorno
    hh <- substr(rawStazioni$DTRF[i],12,13) #ora
    mm <- substr(rawStazioni$DTRF[i],15,16) #minuto
    YYYYMMDDhhmm <- paste0(YYYY,MM,DD,hh,mm)
    
    rawStazioni$DTRF[i] <- YYYYMMDDhhmm
  }
  ...
  n_finestre <- (nrow(rawStazioni)-1)/MisureInFinestra # numero di misure aggregate (ESCLUDO LO "0" DI MISURAZIONE)
  primaMisura = 2 # L'AGGREGAZIONE PARTE CON LA MISURA SUBITO DOPO L'ISTANTE DI MISURA (es. inizio=12:00 --> prima misura 12:10)
  ...
  for(i in 2:ncol(rawStazioni)){ # scansione della finestra indice per ciascuna stazione i
    ...
    rawStatusStaz$nNA[which(rawStatusStaz$ID==colnames(rawStazioni)[i])] <- sum(is.na(rawStazioni[i]))
  ...
  if(filtroNA == T){
    ...
    sum(is.na(rawStatusStaz$Status))

...
CorkDet <- function(GaugesData4CorkDetective, 
  ...
  # Controllo assenza dato radar ____ 
  # se assente eseguirò il solo controllo di consistenza spaziale 
  
  if(list_Radar_aggregated$flag_find_img_radar == 0){
    
    message("WARNING: ASSENTE DATO RADAR, ESEGUIRO' SOLO CONTROLLO CONSISTENZA SPAZIALE! ")
  ...
  #_____
    ...
    # ESTRAZIONE DATO RADAR SUI PLUVIO -------------------------------------------------------------------------------------
    # dai raster radar aggregati sulla finestra estraggo dati puntuali dei pluviometri
    
    # NB: Questa operazione avviene solo con disponibilità di immagini radar
    
    if(list_Radar_aggregated$flag_find_img_radar == 1){
    ...
    } else if(list_Radar_aggregated$flag_find_img_radar == 0){
      
      message(paste0("***** Inizio check: SCC    ",substr(Sys.time(),12,20)))
    ...
    for(i in 1:length(Pluvio)){
      ...
      # NB: RCC dipende da disponibilità immagini radar;
      #     TCC dipende dal fatto che RCC sia stato eseguito. Se immagine radar assente, anche TCC non verra eseguito
